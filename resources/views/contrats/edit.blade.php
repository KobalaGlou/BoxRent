                ...
                <div class="p-6 bg-white border-b border-gray-200">
                    ...
                    <form action="{{ route('contrats.update', $contrat->id) }}" method="POST">
                        ...
                        <div class="mb-4">
                            ...
                            <select name="locataire_id" id="locataire_id" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" required>
                                @foreach ($locataires as $locataire)
                                    <option value="{{ $locataire->id }}" {{ $locataire->id == $contrat->locataire_id ? 'selected' : '' }}>{{ $locataire->nom }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <select name="box_id" id="box_id" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" required>
                                @foreach ($boxes as $box)
                                    <option value="{{ $box->id }}" {{ $box->id == $contrat->box_id ? 'selected' : '' }}>{{ $box->nom }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <input type="date" name="date_debut" id="date_debut" value="{{ $contrat->date_debut }}" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" required>
                        </div>

                        <div class="mb-4">
                            <label for="date_fin" class="block text-gray-700 font-bold mb-2">Date de Fin :</label>
                            <input type="date" name="date_fin" id="date_fin" value="{{ $contrat->date_fin }}" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
                        </div>

                        <div class="mb-4">
                            <label for="prix_mois" class="block text-gray-700 font-bold mb-2">Prix Mensuel (€) :</label>
                            <input type="number" name="prix_mois" id="prix_mois" value="{{ $contrat->prix_mois }}" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" required>
                        </div>
